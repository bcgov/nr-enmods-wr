...
BEGIN
    ...
    -- ...repeat RAISE NOTICE for each index drop...
        ...
        END IF;
        IF EXISTS (SELECT 1 FROM pg_indexes WHERE schemaname='public' AND indexname='idx_aqi_stg_analysis_method_id') THEN
            RAISE NOTICE 'Dropping index: idx_aqi_stg_analysis_method_id';
            EXECUTE 'DROP INDEX public.idx_aqi_stg_analysis_method_id';
        END IF;
    END
    $drop_stg_idx$;
    ...
    RAISE NOTICE 'Creating index: idx_aqi_stg_analysis_method_id';
    EXECUTE format('CREATE INDEX idx_aqi_stg_analysis_method_id                ON %I.%I (Analysis_Method_ID)',                         v_schema, v_stg);
